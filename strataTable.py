...
def create_pdf(pdf, experiment):
    ...
    # Display experiment number (from the form) on the PDF
    pdf.setFont("Helvetica", 12)
    pdf.drawString(50, 800, f"Experiment Number: {experiment}")
...
def main():
    ...
    experiment = form.getfirst("experiment", "N/A")

    # Create PDF in memory
    buffer = io.BytesIO()
    pdf = canvas.Canvas(buffer, pagesize=A4)
    create_pdf(pdf, experiment)
    pdf_data = buffer.getvalue()
    buffer.close()

    # Print HTTP headers and PDF content
    sys.stdout.buffer.write(b"Content-Type: application/pdf\r\n")
    sys.stdout.buffer.write(b"Content-Disposition: inline; filename=strata.pdf\r\n")
